<x-layout>
    ...
    <section class="md:col-span-3">
        ...
        <div class="container mx-auto p-4">
            ...
            <div x-data="{ open: false }" id="applicant-form">
                <button @click="open = true" class="block w-full tet-center px-5 py-2.5 shadow-sm rounded border text-base font-medium
                ...
                <div x-cloak x-show="open" class="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-50">
                    <div @click.away="open = false" class="bg-white p-6 rounded-lg shadow-md w-full max-w-md">
                        <h3 class="text-lg font-semibold md-4">
                            Apply For {{$job->title}}
                        </h3>
                        <form enctype="multipart/form-data" method="POST" action="{{route('applicants.store', $job->id)}}">
                            @csrf
                            <x-inputs.text id="full_name" name="full_name" label="Full Name"  :required="true" />
                            <x-inputs.text id="contact_phone" type="phone" name="contact_phone" label="Contact Phone" />
                        ...
                        py-2 rounded-md">Submit Application</button>
                            <button @click="open=false" class="bg-blue-500 hover:bg-blue-600 text-white px-4
                        py-2 rounded-md">Cancel</button>
                        </form>
                    </div>
                </div>
            </div>
    ...
    <aside class="bg-white rounded-lg shadow-md p-3">
        ...
        <img src="/storage/{{$job->company_logo}}"
            alt="Ad"  class="w-full rounded-lg mb-4 m-auto"/>
        @endif
        ...
        @else
            <form method="POST" action="{{auth()->user()->bookmarkedJobs()->where('job_id',$job->id)
                ->exists() ? route('bookmarks.destroy', $job->id) : route('bookmarks.store', $job->id)}}" class="mt-10">
                @csrf
                @if(auth()->user()->bookmarkedJobs()->where('job_id',$job->id)->exists())
                    @method('DELETE')
                    <button class="bg-red-500 hover:bg-red-600 text-white font-bold w-full py-2
